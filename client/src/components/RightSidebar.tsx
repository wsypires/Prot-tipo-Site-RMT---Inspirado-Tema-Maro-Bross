...
const quickLinks: QuickLink[] = [
  { label: "CHAT", icon: <MessageCircle size={14} />, path: "/chat" },
  { label: "TOKENS", icon: <DollarSign size={14} />, path: "/buy-tokens" },
  { label: "DENUNCIAR", icon: <AlertTriangle size={14} />, path: "/reports" },
  { label: "SUPORTE", icon: <HelpCircle size={14} />, path: "/support" },
];
    ...
    <aside className="hidden xl:block fixed right-0 top-0 h-screen w-80 z-30">
      <div className="relative h-full flex flex-col gap-3 p-3 overflow-y-auto border-l-4 border-mario-black" style={{ backgroundColor: 'var(--mario-sky)' }}>
        ...
        <div className="mario-panel bg-mario-green text-mario-white">
          <div className="flex items-center gap-2 mb-3 pb-2 border-b-2 border-mario-black">
            <Zap size={16} />
            <h3 className="text-xs font-bold">LINKS RÁPIDOS</h3>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {quickLinks.map((link) => (
              <button
                ...
                className="p-2 bg-mario-white text-mario-black border-2 border-mario-black hover:bg-mario-yellow transition-all flex flex-col items-center gap-1"
              >
                {link.icon}
                <p className="text-xs font-bold text-center">{link.label}</p>
              </button>
            ))}
          </div>
        </div>
